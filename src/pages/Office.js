...
const Office = () => {
  ...
  useEffect(()=>{
    const fakeTime=[{name:"Eric",arrive_time:"00:00:00",leave_time:"12:00:00"},{name:"Amanda",arrive_time:"00:00:00",leave_time:"12:00:00"},{name:"Amy",arrive_time:"00:00:00",leave_time:"12:00:00"},]
    setRows(fakeTime);
  },[])
  ...
  const handleLeave = () =>{
    ...
    const arr=rows.filter((row)=>{return row.name!=="Emily"})
    const leave ={name:"Emily",arrive_time:"09:00:00",leave_time:"14:00:00"};
  ...
  return (
    ...
    <Button className="signBtn" onClick={arrive ? handleLeave : handleArrive }>{arrive ? "下班":"上班"}</Button>
    </div>
